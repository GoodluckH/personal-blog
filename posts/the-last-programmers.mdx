...
Think about the money flowing into AI development right now. Billions of dollars from the smartest investors in the world, all betting that AI coding will get dramatically better. Compute will get faster.

Today, we already have near instant voice-to-text capabilities, tomorrow, that's going to be voice-to-code.

You sit down, describe what you want in plain English, and working software appears instantly. The only friction between your idea and reality is how clearly you can explain what you want to build.
...
His setup is wild to see in person. He has six [Claude Code](https://www.anthropic.com/claude-code) terminal windows open at once, each one handling a different task or feature. He literally speaks to them one by one using [Whispr Flow](https://wisprflow.ai/), and they all execute in parallel. Most of his day is spent reviewing design documents and looking at the actual web app to see the changes being made in real time. Only in very rare cases does he actually dive into the code to debug something. And as model quality improves, I'm convinced those rare debugging sessions will disappear entirely.
